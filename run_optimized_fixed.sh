...
# Script to run the optimized ADRVE Edge Device (fixed version)
# Usage: ./run_optimized_fixed.sh [--profile PROFILE_NAME] [--no-display] [--rtsp-url URL]
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    *)
      ...
      echo "Usage: ./run_optimized_fixed.sh [--profile PROFILE_NAME] [--no-display] [--rtsp-url URL]"
      exit 1
      ;;
  esac
done
...
echo "Running optimized edge device script (fixed version)..."
python edge-device-optimized-fixed.py --profile $PROFILE $NO_DISPLAY --rtsp-url "$RTSP_URL"
