...
def get_kvs_data_endpoint(stream_name):
...
def extract_frame_from_kvs(stream_name):
    ...
    try:
        ...
        endpoint = get_kvs_data_endpoint(stream_name)
        
        # Create a client using the endpoint
        kvs_client = boto3.client('kinesisvideo', endpoint_url=endpoint, region_name='us-west-2')
        ...
        # Read the payload
        payload = response['Payload'].read(1024*1024)  # Read 1MB of data
        ...
        try:
            ...
            if frame is None:
                print("Failed to decode frame from KVS data")
                return None
            return frame
