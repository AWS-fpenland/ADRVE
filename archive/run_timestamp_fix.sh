...
# Script to run the ADRVE Edge Device with timestamp fix
# Usage: ./run_timestamp_fix.sh [--profile PROFILE_NAME] [--no-display] [--rtsp-url URL]
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    *)
      ...
      echo "Usage: ./run_timestamp_fix.sh [--profile PROFILE_NAME] [--no-display] [--rtsp-url URL]"
      exit 1
      ;;
  esac
done
...
# Run the edge device script with timestamp fix
echo "Running edge device script with timestamp fix..."
python edge-device-timestamp-fix.py --profile $PROFILE $NO_DISPLAY --rtsp-url "$RTSP_URL"
