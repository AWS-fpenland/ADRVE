...
# Script to run the ADRVE Edge Device in headless mode
# Usage: ./run_headless.sh [--profile PROFILE_NAME] [--no-yolo] [--rtsp-url URL]
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    *)
      ...
      echo "Usage: ./run_headless.sh [--profile PROFILE_NAME] [--no-yolo] [--rtsp-url URL]"
      exit 1
      ;;
  esac
done
...
echo "YOLO Detection: $([ -z "$NO_YOLO" ] && echo "Enabled" || echo "Disabled")"
...
# Run the edge device script with the specified parameters
echo "Running edge device script in headless mode..."
python edge-device-headless.py --profile $PROFILE $NO_YOLO --rtsp-url "$RTSP_URL"
