#!/usr/bin/env python3
"""
ADRVE Edge Device Script with GPU Acceleration (Fixed)
---------------------------------------------
...
Usage:
  python edge-device-script-gpu-fixed.py [--profile PROFILE_NAME]
...
def start_kvs_producer():
    ...
    try:
        ...
        # Method 2: Use GPU decoding and encoding
        # This is useful if you want to process the video frames or change resolution/format
        kvs_gpu_command = [
            "gst-launch-1.0", "-v",
            "rtspsrc", f"location={rtsp_url}", "short-header=TRUE", "!",
            "rtph264depay", "!", "h264parse", "!",
        ...
        try:
            ...
            if process.poll() is not None:
                ...
                # Try Method 2 if Method 1 fails (GPU decoding and encoding)
                print("Method 1 failed. Trying Method 2 with GPU decoding and encoding...")
                process = subprocess.Popen(
                    kvs_gpu_command,
                    stdout=subprocess.PIPE,
                    stderr=subprocess.PIPE,
