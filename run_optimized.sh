...
# Script to run the optimized ADRVE Edge Device
# Usage: ./run_optimized.sh [--profile PROFILE_NAME] [--no-display] [--rtsp-url URL]
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    *)
      ...
      echo "Usage: ./run_optimized.sh [--profile PROFILE_NAME] [--no-display] [--rtsp-url URL]"
      exit 1
      ;;
  esac
done
...
echo "Running optimized edge device script..."
python edge-device-optimized.py --profile $PROFILE $NO_DISPLAY --rtsp-url "$RTSP_URL"
