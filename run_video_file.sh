...
# Script to run the ADRVE Edge Device with a video file input
# Usage: ./run_video_file.sh --video-file FILE.mkv [--profile PROFILE_NAME] [--no-display]
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    *)
      ...
      echo "Usage: ./run_video_file.sh --video-file FILE.mkv [--profile PROFILE_NAME] [--no-display]"
      exit 1
      ;;
  esac
done
...
# Run the edge device script with video file
echo "Running edge device script with video file input..."
python edge-device-video-file.py --profile $PROFILE $NO_DISPLAY --video-file "$VIDEO_FILE"
