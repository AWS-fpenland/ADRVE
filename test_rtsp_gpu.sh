...
# Test script to stream RTSP to KVS using NVIDIA GPU acceleration
# Usage: ./test_rtsp_gpu.sh [rtsp_url]
...
echo "Starting GStreamer pipeline with GPU acceleration to stream RTSP to KVS..."
echo "RTSP URL: $RTSP_URL"
echo "KVS Stream: $STREAM_NAME"

# Try different GPU acceleration methods in order of preference

# Method 1: Using nvh264enc (NVIDIA hardware encoder)
echo "Method 1: Using nvh264enc (NVIDIA hardware encoder)..."
...
if [ $? -ne 0 ]; then
    echo "Method 1 failed. Trying Method 2: Using nvenc..."
    gst-launch-1.0 -v rtspsrc location="$RTSP_URL" short-header=TRUE ! \
        rtph264depay ! h264parse ! \
        nvenc ! \
        h264parse ! \
...
if [ $? -ne 0 ]; then
    echo "Method 2 failed. Trying Method 3: Using vaapih264enc..."
    gst-launch-1.0 -v rtspsrc location="$RTSP_URL" short-header=TRUE ! \
        rtph264depay ! h264parse ! \
        vaapih264enc ! \
        h264parse ! \
...
# If Method 3 fails, try Method 4 (no transcoding, pass-through)
if [ $? -ne 0 ]; then
    echo "Method 3 failed. Trying Method 4: Direct pass-through (no transcoding)..."
    gst-launch-1.0 -v rtspsrc location="$RTSP_URL" short-header=TRUE ! \
        rtph264depay ! h264parse ! \
        video/x-h264,stream-format=avc,alignment=au ! \
        kvssink stream-name="$STREAM_NAME" storage-size=128
...
# If all methods fail, try the KVS GStreamer sample
if [ $? -ne 0 ]; then
    echo "All GStreamer pipeline methods failed. Trying KVS GStreamer sample..."
