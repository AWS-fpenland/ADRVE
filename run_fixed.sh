...
# Script to run the fixed version of the ADRVE Edge Device with a video file input
# Usage: ./run_fixed.sh --video-file FILE.mkv [--profile PROFILE_NAME] [--no-display]
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    *)
      ...
      echo "Usage: ./run_fixed.sh --video-file FILE.mkv [--profile PROFILE_NAME] [--no-display]"
      exit 1
      ;;
  esac
done
...
# Run the fixed edge device script with video file
echo "Running fixed edge device script with video file input..."
python edge-device-video-file-fixed.py --profile $PROFILE $NO_DISPLAY --video-file "$VIDEO_FILE"
